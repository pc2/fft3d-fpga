...
TEST_F(fftFPGATest, ValidSp2dFFT){
  ...
  float2 *out = (float2*)fftfpgaf_complex_malloc(sz, 0);
  // null inp ptr input
  fpga_t fft_time = fftfpgaf_c2c_2d(64, NULL, out, 0);
  EXPECT_EQ(fft_time.valid, 0);

  // null out ptr input
  fft_time = fftfpgaf_c2c_2d(64, inp, NULL, 0);
  EXPECT_EQ(fft_time.valid, 0);

  // if N not a power of 2
  fft_time = fftfpgaf_c2c_2d(63, inp, out, 0);
  EXPECT_EQ(fft_time.valid, 0);
  ...
  int test = fpga_initialize("Intel(R) FPGA", "64pt_fft2d_emulate.aocx", 0, 1);
  ASSERT_NE(test, 1);

  fft_time = fftfpgaf_c2c_2d(64, inp, out, 0);
