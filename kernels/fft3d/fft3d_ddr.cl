 ...
 const unsigned N = (1 << LOGN);
  ...
  for(unsigned k = 0 ; k < N; k++){
   ...
   for(unsigned row = 0; row < N; row++){
      ...
      for(unsigned j = 0; j < (N / 8); j++){
        bitrev[j] = read_channel_intel(chanoutfft1[0]);               // 0
        bitrev[4 * N / 8 + j] = read_channel_intel(chanoutfft1[1]);   // 32
        bitrev[2 * N / 8 + j] = read_channel_intel(chanoutfft1[2]);   // 16
        bitrev[6 * N / 8 + j] = read_channel_intel(chanoutfft1[3]);   // 48
        bitrev[N / 8 + j] = read_channel_intel(chanoutfft1[4]);       // 8
        bitrev[5 * N / 8 + j] = read_channel_intel(chanoutfft1[5]);   // 40
        bitrev[3 * N / 8 + j] = read_channel_intel(chanoutfft1[6]);   // 24
        bitrev[7 * N / 8 + j] = read_channel_intel(chanoutfft1[7]);   // 54
      }
 ...
 const unsigned N = (1 << LOGN);
  ...
  for(unsigned k = 0 ; k < N; k++){
   ...
   for(unsigned row = 0; row < N; row++){
      ...
      // bit-reversed ordered input stored in normal order
      for(unsigned j = 0; j < (N / 8); j++){
        bitrev[j] = read_channel_intel(chaninfft2[0]);               // 0
        bitrev[4 * N / 8 + j] = read_channel_intel(chaninfft2[1]);   // 32
        bitrev[2 * N / 8 + j] = read_channel_intel(chaninfft2[2]);   // 16
        bitrev[6 * N / 8 + j] = read_channel_intel(chaninfft2[3]);   // 48
        bitrev[N / 8 + j] = read_channel_intel(chaninfft2[4]);       // 8
        bitrev[5 * N / 8 + j] = read_channel_intel(chaninfft2[5]);   // 40
        bitrev[3 * N / 8 + j] = read_channel_intel(chaninfft2[6]);   // 24
        bitrev[7 * N / 8 + j] = read_channel_intel(chaninfft2[7]);   // 54
      }
    ...
    for(unsigned row = 0; row < N; row++){
      ...
      for(unsigned j = 0; j < N / 8; j++){
        ...
        write_channel_intel(chanoutstore1[0], rotate_out[chan0]);    // 0
        write_channel_intel(chanoutstore1[1], rotate_out[chan1]);   // 32
        write_channel_intel(chanoutstore1[2], rotate_out[chan2]);   // 16
        write_channel_intel(chanoutstore1[3], rotate_out[chan3]);   // 48
        write_channel_intel(chanoutstore1[4], rotate_out[chan4]);   // 8
        write_channel_intel(chanoutstore1[5], rotate_out[chan5]);   // 40
        write_channel_intel(chanoutstore1[6], rotate_out[chan6]);   // 24
        write_channel_intel(chanoutstore1[7], rotate_out[chan7]);   // 54
      }
    } // row
